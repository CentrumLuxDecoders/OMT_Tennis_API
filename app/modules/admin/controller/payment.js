...
//Payment code by Kavin ----Start------------
exports.payments=async function(req, res, next) {
  var _output = new output();
  //console.log('req body '+ JSON.stringify(req.body));
  const stripe = require("stripe")(
    "sk_test_51HHX7vCieC0KYCQXJo1yWIw6FbqDyWodyGoHRTCfN85X7fdmcJbHMKOIpLDkVoBDXXjbpK2OW4dm7jfp3pSjbkuG00zi7INE6y"
  );

    const btokId = req.body.token;
console.log(req.body);
    stripe.customers.create(
      {
      ...
      (err, customer) => {
        ...
        if(err){
            _output.data = err;
            _output.isSuccess = false;
            _output.message = err.raw.message;
            res.send(_output);
        }else{
         ...
         stripe.charges.create({
              ...
              currency: 'EUR',
              description: req.body.email +" Paiement success" ,
         },(err,charges)=> {
            if(err){
              _output.data = err;
              _output.isSuccess = false;
              ...
              _output.data = "";
              _output.isSuccess = true;
              _output.message = "Paiement success";
              res.send(_output);
