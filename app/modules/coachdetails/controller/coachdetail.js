...
exports.bookCourse = async function(req, res, next) {  
  ...
  if (
    ...
    bookingEnd != "" &&
    course != "" && 
    P_BookingCourseID != "" && P_BookingCourseID != undefined  
  ) {
     
    if (amount == "" || amount == undefined) {
     
      var insert_qry =
        "INSERT INTO `booking_dbs`(`bookingCourseID`,`Coach_ID`, `user_Id`, `status`, `bookingDate`, `bookingEnd`, `bookingCourse`, `amount`) " +
        "VALUES ('"+ P_BookingCourseID + "'," +
        Coach_id +
        "," +
        user_Id +
        ",'" +
      ...
      await db_library
        ...
        console.log(reserve_qry);
            ...
            await db_library
              ...
              .then(async values => {
                if (values.affectedRows > 0) {
                  await db_library
                    ...
                    .catch(err => {
                     console.log("line - 241",err.message);
                      _output.data = {};
                      _output.isSuccess = false;
              ...
              .catch(err => {
                console.log("line 249",err.message);
                _output.data = {};
                _output.isSuccess = false;
                _output.message = "Booking Inserted Failed";
              });
...
exports.search_for_event_top_3 = async function(req, res, next) {
  ...
  if (P_course == "Stage") {
    var query =
      "SELECT `Eventname`, `from_date`, `to_date`, `Description`, `Postalcode`, `Photo` FROM `course_stage` WHERE (('" +
      P_date +
      "' BETWEEN `from_date` AND `to_date`) or '" +
      P_date +
      "' = '0000-00-00'or '" +
...
if (P_course == "Stage") {
    query =
      "SELECT cd.*,ci.coordonnees_gps,ci.Libelle_acheminement as cityname FROM `course_stage` cd INNER JOIN `cities` ci on ci.Code_postal = cd.Postalcode AND ci.Code_commune_INSEE= cd.Coach_Ville where from_date >= '" +
      formatDate(new Date()) +
      "';";
  } else if (P_course == "Tournament") {
    query =
      "SELECT cd.*,ci.coordonnees_gps,ci.Libelle_acheminement as cityname FROM `tournament` cd INNER JOIN `cities` ci on ci.Code_postal = cd.Postalcode AND ci.Code_commune_INSEE= cd.Coach_Ville where from_date >= '" +
      formatDate(new Date()) +
      "';";
  } else if (P_course == "Animation") {
    query =
      "SELECT cd.*,ci.coordonnees_gps,ci.Libelle_acheminement as cityname FROM `animations` cd INNER JOIN `cities` ci on ci.Code_postal = cd.Postalcode AND ci.Code_commune_INSEE= cd.Coach_Ville ;";
  } else {
    query = "SELECT cd.*,ci.coordonnees_gps,ci.Libelle_acheminement as cityname FROM `team_building`  cd INNER JOIN `cities` ci on ci.Code_postal = cd.Postalcode AND ci.Code_commune_INSEE= cd.Coach_Ville ;";
  }
...
exports.getCoachbyevent = async function(req, res, next) {
  ...
  if(UserId != undefined && UserId != '' && UserId != null){
   var query1 = "SELECT * from booking_dbs where Coach_ID = " + P_CoachId + "  AND user_id = " + UserId + " limit 1"
